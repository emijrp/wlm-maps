...
try {
	$sql="SELECT country, lang, id, name, lat, lon, image, monument_article, monument_random FROM monuments_all WHERE lon>=:left AND lon<=:right AND lat>=:bottom AND lat<=:top ORDER BY monument_random LIMIT 200";
	$stmt = $db->prepare($sql);
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($prop['image']) {
        $image_ = str_replace(' ', '_', $prop['image']);
        ...
        $image_md5 = md5('Image_upload-tango.svg');
        $prop['image_url']=$prop['upload_url'];
        $prop['thumb_url']='https://upload.wikimedia.org/wikipedia/commons/thumb/'.substr($image_md5,0,1).'/'.substr($image_md5,0,2).'/Image_upload-tango.svg/150px-Image_upload-tango.svg.png';
    }
